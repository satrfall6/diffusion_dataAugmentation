...
from denoising_diffusion_pytorch.denoising_diffusion_pytorch_myUnetCond import Unet, GaussianDiffusion
...
from dataset_abs import random_head_diffusion_loader, random_head_diffusion_loader_reduce_ch
...
parser.add_argument('--test_obj', type=str, default='diff_myUnet_abs_GaussNorm', help='name for separating wandb project')
parser.add_argument('--time_steps', type=int, default=500, help='sampling time steps')

parser.add_argument('--n_epochs', type=int, default=180, help='number of epochs of training')
parser.add_argument('--batchSize_train', type=int, default=2, help='size of the batches')
...
parser.add_argument('--in_dim', type=int, default=16, help='base dim of model input')
parser.add_argument('--if_train', type=bool, default=False, help='if train only')
parser.add_argument('--if_test', type=bool, default=False, help='if test only')


parser.add_argument('--lr', type=float, default=0.00001, help='initial learning rate of G')
...
def check_results(generated_signals, loc_labels, norm, epoch, reduced_idx, exp_id=None, save_dir=None):
    ...
    for i, b in enumerate(range(generated_signals.shape[0])):
        ...
        generated_signal = generated_signal * torch.tensor([0.247]) + torch.tensor([0.4822]) 
        ...
        if generated_signal.shape[-1] != 256:
            ...
            denormed_signal_zeroFill = denormed_signal


        confocal_result = DMAS_updated(denormed_signal_zeroFill, False)
        # for plotring loc label
        ...
        axes[1].axis('off')
        try:
            axes[1].set_title(f'Confocal Result {exp_id[b]}')
        except:
            axes[1].set_title('Confocal Result')
        
        # Plot generated siganl
        im2 = axes[2].imshow(generated_signal, cmap='gray')  # Adjust colormap as needed
        ...
        if save_dir is not None:
            try:
                ...
                plt.savefig(save_dir / f'Epoch{epoch}_{b}.png')
            plt.close()
        else:
            plt.show()
...
def sparse_to_indices(sparse_labels):
    indices = [torch.nonzero(labels, as_tuple=False).squeeze(1).cuda() for labels in sparse_labels]
    return indices           
#%% load dataset
...
if opt.img_size == 256:
    train_dataset = random_head_diffusion_loader(train_total_paths, train_healthy_paths,
                                                 loc_label_path)
    norm_tr = train_dataset.norm_values_tr
    norm_tt = train_dataset.norm_values_tt
    test_dataset = random_head_diffusion_loader(test_total_paths, test_healthy_paths,
                                                loc_label_path,
                                                norm_tt, norm_tr)
else:
...
# plt.imshow(batch_signals[0], cmap='gray')
# plt.colorbar()
#%%
num_classes = 103
model = Unet(
...
diffusion = GaussianDiffusion(
    ...
    timesteps = opt.time_steps    # number of steps
).cuda()
...
if opt.if_train:
    ...
    for epoch in range(opt.n_epochs ):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            loc_labels = batch['loc_label'].float()
            indices_labels = sparse_to_indices(loc_labels)
            ############3 check signals ###########
            # signals = batch['tr'].float()
            # plt.imshow(signals[0,0])
            # plt.title(f'max:{round(signals[0,0].max().item(),4)}, min:{round(signals[0,0].min().item(), 4)}')
            # plt.colorbar()
            # plt.show()
        ...
        #%%
            optimizer.zero_grad()
            loss = diffusion(signals, indices_labels)  # Forward pass and loss computation
            loss.backward()  # Backward pass
            optimizer.step()  # Update model parameters
    
            # Weight clipping
            for p in model.parameters():
                p.data.clamp_(-1, 1)  # Clipping the weights to the range [-0.01, 0.01]
        ...
        if epoch % 10 == 0:
            ...
            sampled_images = diffusion.sample(indices_labels, batch_size=loc_labels.shape[0]
            ...
            check_results(sampled_images, loc_labels, norm_tr, epoch, reduced_dim_idx,
                          save_dir=plot_path)
...
if opt.if_test:
    ...
    diffusion.load_state_dict(torch.load(final_weights_path/f'diffusion{opt.test_obj}.pth'))
    # diffusion.load_state_dict(torch.load(final_weights_path/f'diffusion{opt.test_obj}.pth', map_location=torch.device('cpu')))
    sampled_images = diffusion.sample(batch_size=opt.batchSize_test
    ).detach().cpu().squeeze(1)
    epoch = 'test'
    plot_path = root_path / f'plots/{opt.test_obj}'
