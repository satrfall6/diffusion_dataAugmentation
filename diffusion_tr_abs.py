...
# from denoising_diffusion_pytorch.denoising_diffusion_pytorch_myUnet import Unet, GaussianDiffusion
...
def check_results(generated_signals, norm, epoch, reduced_idx, exp_id=None, save_dir=None):
    ...
    for i, b in enumerate(range(generated_signals.shape[0])):
        ...
        axes[0].axis('off')
        try:
            axes[0].set_title(f'Confocal Result {exp_id[b]}')
        except:
            axes[0].set_title('Confocal Result')
        
        # Plot generated siganl
        im2 = axes[1].imshow(generated_signal, cmap='gray')  # Adjust colormap as needed
